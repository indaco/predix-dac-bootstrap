...
/* PredixDACDetails
predix-app-composer.$predixZone.user
*/
case class PredixDACDetails(
  label: String,
  name: String,
  plan: String,
  uri: String,
...
case class UAAClient(
  ...
  def toJsonString: String = {
    ...
    s""" {"client_id": "$clientId", "client_secret": "$clientSecret", "name": "$clientId", "authorized_grant_types": $authorizedGrantTypesJson, "allowedproviders": $allowProvidersJson ,"authorities": $authoritiesJson, "scope": $scopesJson, "redirect_uri": $redirectUriJson, "autoapprove": $autoApproveJson, "access_token_validity": $accessTokenValidity, "refresh_token_validity": $refreshTokenValidity, "redirect_uri": [ "http://test1.com" ]} """
  }
}
