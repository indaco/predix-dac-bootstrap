...
def main() = {
  ...
  // Logger setup
  val appLogger = new LoggerUtils(LOGS_FOLDER, "predix-dacdemo.log")
  ...
  // Create DAC Service
  appLogger.showAndAppendToLogFile("info", s"\n * Create service if it does not exist: $DAC_SERVICE_NAME")
  PredixUtils.createDACService(
    DAC_SERVICE_NAME,
    DAC_SERVICE_PLAN,
    DAC_INSTANCE_NAME,
    uaaIssuerId) match {
  ...
  // Binding Sample App to DAC Service
  appLogger.showAndAppendToLogFile("info", s"\n * Binding $DAC_INSTANCE_NAME to $FRONT_END_APP_NAME...")
  PredixUtils.cfBind(FRONT_END_APP_NAME, DAC_INSTANCE_NAME) match {
    case Some(s) => StringUtils.checkCommandResultString(s, appLogger)
    case None => System.exit(-1)
  }
  ...
  var dacDetails = None: Option[PredixDACDetails]
  var configJsonFileContent = None: Option[String]

  // Getting app's ENV variables for TimeSeries, Asset, SQL and DAC
  appLogger.showAndAppendToLogFile("info", s"\n * Getting VCAP for $FRONT_END_APP_NAME ...")
  PredixUtils.getVCAPJson(FRONT_END_APP_NAME) match {
    case Some(s) =>
      val u = upickle.json.read(s)(0)
      ...
      dacDetails = Some( (PredixDACDetails.apply _).tupled(PredixUtils.getDACConfigfromVCAP(u)) )
  ...
  // Show app's ENV variables for DAC
  appLogger.showAndAppendToLogFile("info", "** Predix DAC Details:")
  appLogger.showAndAppendToLogFile("info", dacDetails.getOrElse(EMPTY_STRING).toString)
  ...
  val createClientRes: Map[String, String] =
    UAAUtils.addClient(uaaDetails.get, UAA_CLIENTID_GENERIC, UAA_CLIENTID_GENERIC_SECRET, tsDetails.get, assetDetails.get, dacDetails.get, adminToken("token"), appLogger)
  ...
  // Close the script
  appLogger.showAndAppendToLogFile("info", "\n===========================================================================")
  appLogger.showAndAppendToLogFile("info", "\n*** DONE! Log file saved as: " + pwd/'logs/"predix-dacdemo.log")
  appLogger.showAndAppendToLogFile("info", "\n===========================================================================\n")
